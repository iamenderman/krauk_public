...
// writes hash of to_be_hashed to fd
void write_hash(FILE *fp, file_info to_be_hashed, uint16_t *non_hashables);
...
int krauk_post(KRAUK_FD kc, CLIENT_CTX *ctx, uint32_t repo_id, char *msg) {
    ...
    // sends the tracked compressed files
...
void write_hash(FILE *fp, file_info to_be_hashed, uint16_t *non_hashables) {
    ...
    TODO: check if new sha1 library is better
        sha1 lib got bugs when hashing large files -> max size for hashing is introduces ~1.25mb
        this needs to be fixed!!
    */
    if (to_be_hashed.file_s.st_size + 1 > 10000000) {
    ...
    SHA1(to_be_hashed.file, to_be_hashed.file_s.st_size + 1, res);
...
FIlE_TYPE_TABLE rewrite_file_hashes() {
    ...
    // eternal hash managing
    uint16_t non_hashables = 0;

    // open and parses tracked file
    track_file = open_file(C_TRACKED_FILE, O_RDONLY, MEM_COPY);
    ...
    for (size_t c = 0; c < track_file_rowifed.row_count; c += 2) {
        ...
        to_be_hashed = open_file(track_file_rowifed.rows[c], O_RDONLY, MEM_DEFAULT);

        // manages empty files
        if (to_be_hashed.file_s.st_size == 0) {
            fprintf(fp, "%s", KRAUK_EMPTY_FILE);
        } else {
            // writes the hash of 'to_be_hashed' to file_d;
            write_hash(fp, to_be_hashed, &non_hashables);
            // creates freq table and adds it to the file type table
            fill_file_freq_table(to_be_hashed, table);

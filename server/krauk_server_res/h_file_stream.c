...
int handle_hased_file_stream(KRAUK_FD ksc, CLIENT_CTX *ctx, uint8_t *buffer) {
    ...
    // freqtable setup
    ft_table = FIlE_TYPE_TABLE_deseralize_table(freq_table_file.file);
    ...
    archive_unpack_file(temp_table, hashed, GOBAL_TEMP_FILE);
    temp_file = open_file(GOBAL_TEMP_FILE, O_RDWR, MEM_DEFAULT);

    // sends file to client
    krauk_send_file(ksc, ctx, temp_file);

    // cleanup
    if (remove(GOBAL_TEMP_FILE) == -1) {
...
int handle_file_stream(KRAUK_FD ksc, CLIENT_CTX *ctx, uint8_t *buffer) {
    ...
    if (response[1] == REQUEST_ERROR) {
        puts("[!] Client requsted non-existing file");
        return -1;
    }
